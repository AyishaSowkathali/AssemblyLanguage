...
####################(8)####################
					addi $a0, $sp, 94
					jal GetOneIntByVal
					lw $t3, 49($sp)		# valsToDo
					sub $t0, $t3, $t1		
					sll $t0, $t0, 2		# valsToDo - i
					addi $t3, $sp, 1		# $t3 has address of intArr[]
					add $t3, $t3, $t0	
					sw $v0, 0($t3)		# store value into intArr[]
					j endI1
#         else // i is even
ElseI1:
...
####################(3)####################
					li $t1, 0		# $t2 is i
					lw $t0, 49($sp)	#  
					addi $t2, $t0, -1	# $t2 is j and has valsToDo - 1	
					j FTestM2
begFBodyM2:
...
####################(5)####################
					addi $t3, $sp, 1	# $t3 has address of intArr[ ]
					sll $t0, $t1, 2
					add $a0, $t3, $t0	# add addr and i
					sll $t0, $t2, 2
					add $a1, $t3, $t0	# add addr and j
...
# Register Usage:
#################
# $t0: value of intPtr1 ($a0) 
# $v1: value loaded from  intPtr2 ($a1)
#################
#   int temp = *intPtr1;
#  *intPtr1 = *intPtr2;
#   *intPtr2 = temp;
...
# Register Usage:
#################
# $t3: holds addr of charPtr (to avoid clobbering)
#################
#   cout << prompt;
#   cin >> *charVarToPutInPtr;

####################(7)####################
					move $t3, $a0		#protect $a0 
					move $a0, $a1
					li $v0, 4
					syscall			#print char prompt
					li $v0, 12
					syscall			#read char
					sb $v0, 0($t3)		#store char to addr
#}
					jr $ra
