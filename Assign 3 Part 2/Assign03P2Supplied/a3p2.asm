...
#                      PopulateArray12(12, a1, a2, &used1, &used2);
			...
			####################(6)####################			
			li $a0, 12		# 1st_arg : 12 via register
			addi $a1, $sp, 232		# 2nd_arg : &a1[0] via register
			addi $a2, $sp, 280		# 3rd_arg : &a2[0] via register
			addi $a3, $sp, 228		# 3rd_arg : &used1 via register
			addi $t0, $sp, 224		# 3rd_arg : &used2 via register			
			jal PopulateArray12
#                      cout << endl;
			li $a0, '\n'
			li $v0, 11
...
#                         minInt = PopulateArray34(a1, a2, a3, a4, used1, used2, &used3, &used4);
			...
			####################(12)####################
			addi $a0, $sp, 232		# 1st_arg : &a1[0] via register
			addi $a1, $sp, 280		# 2nd_arg : &a2[0] via register
			addi $a2, $sp, 328		# 3rd_arg : &a3[0] via register
			addi $a3, $sp, 376		# 4th_arg : &a4[0] via register
			lw $t0, 228($sp)		# 5th_arg : $t0 has used1
			lw $t1, 224($sp)		# 6th_arg : $t1 has used2
			addi $t2, $sp, 220		# 7th_arg : &used3 via register
			addi $t3, $sp, 216		# 8th_arg : &used4 via register
			jal PopulateArray34
			move $s0, $v0		# function value stored in minInt
#                      goto endI1_m;
			j endI1_m
#//                    }
else1_m:#//            else
...
#                         ProcArraysA(a1, a2, a3, a4, &used1, &used2, used3, used4);
			...
			addi $a2, $sp, 328		# 3rd_arg : &a3[0] via register
			addi $a3, $sp, 376		# 4th_arg : &a4[0] via register
			addi $t0, $sp, 228		# 5th_arg : &used1 via register
			addi $t1, $sp, 224		# 6th_arg : &used2 via register
			lw $t2, 220($sp)		# 7th_arg : $t2 has used3
			lw $t3, 216($sp)		# 8th_arg : $t3 has used4
			jal ProcArraysA
...
#                         ProcArraysB(minInt, a1, a2, a3, a4, used1, used2, &used3, &used4);
			...
			####################(14)####################
			move $a0, $s0		# 1st_arg : $a0 has the callee saved register minInt
			addi $a1, $sp, 232		# 2nd_arg : &a1[0] via register
			addi $a2, $sp, 280		# 3rd_arg : &a2[0] via register
			addi $a3, $sp, 328		# 4th_arg : &a3[0] via register
			addi $t0, $sp, 376		# 5th_arg : &a4[0] via register
			lw $t1, 228($sp)		# 6th_arg : $t1 has used1
			lw $t2, 224($sp)		# 7th_arg : $t2 has used2
			addi $t3, $sp, 220		# 8th_arg : &used3 via register
			addi $t4, $sp, 216		# 9th_arg : &used4 via register
			jal ProcArraysB
endI2_m:#//            }
...
# PROLOG:
			...
			sw $a1, 4($sp)				# save $a1 (= size1) in caller's frame
			sw $a2, 8($sp)				# save $a2 (= &a2[0]) in caller's frame
			sw $a3, 12($sp)				# save $a3 (= size2) in caller's frame
# BODY:
#                   *used1Ptr = 0;
			lw $t0, 16($sp)
			sw $0, 0($t0)
